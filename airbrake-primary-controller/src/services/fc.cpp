...
static void fc_update_flags(const svc::FusedAlt &f, uint32_t dt_ms) {
  ...
  imu1_reading_t u1; bool v1 = imu1Get(u1) && u1.valid;
  imu2_reading_t u2; bool v2 = imu2Get(u2) && u2.valid;
  ...
  auto upd = [](bool sample_ok, bool &ok, uint32_t &good_acc, uint32_t &bad_acc, uint32_t dt_ms_arg) {
    if (sample_ok) { good_acc += dt_ms_arg; bad_acc = 0; if (!ok && good_acc >= FC_SENSOR_RECOVERY_MS) ok = true; }
    else          { bad_acc  += dt_ms_arg; good_acc= 0; if (ok && bad_acc  >= FC_SENSOR_INVALID_MS)  ok = false; }
  };
  ...
  if (!isnan(tilt)) {
    if (tilt >= FC_TILT_ABORT_DEG) {
      ...
      if (tilt_bad_acc_ms >= FC_TILT_ABORT_DWELL_MS) tilt_latched = true;
    } else {
  ...
  if (!isnan(vz)) {
    ...
    if (mach < on_th) {
      ...
      if (!mach_ok_state && mach_ok_acc_ms >= FC_MACH_DWELL_MS) mach_ok_state = true;
    } else if (mach > off_th) {
      mach_ok_acc_ms = 0;
      mach_ok_state = false;
    }
    if (mach_ok_state) s_flags |= FCF_MACH_OK; else s_flags &= ~FCF_MACH_OK;
  }
  ...
  if (imu1_ok) s_flags |= FCF_SENS_IMU1_OK; else s_flags &= ~FCF_SENS_IMU1_OK;
  if (bmp1_ok) s_flags |= FCF_SENS_BMP1_OK; else s_flags &= ~FCF_SENS_BMP1_OK;
  if (imu2_ok) s_flags |= FCF_SENS_IMU2_OK; else s_flags &= ~FCF_SENS_IMU2_OK;
  if (!tilt_latched && !isnan(tilt) && tilt <= FC_TILT_ABORT_DEG) s_flags |= FCF_TILT_OK; else s_flags &= ~FCF_TILT_OK;
  if (tilt_latched) s_flags |= FCF_TILT_LATCH; else s_flags &= ~FCF_TILT_LATCH;
...
static void fc_update_fsm(const svc::FusedAlt &f, uint32_t now_ms, uint32_t dt_ms) {
  ...
  if (!isnan(f.vz_fused_mps) && f.vz_fused_mps > FC_VZ_LIFTOFF_MPS) liftoff_cond = true;
  if (!isnan(f.az_imu1_mps2) && f.az_imu1_mps2 > FC_AZ_LIFTOFF_MPS2) liftoff_cond = true;
  if (!isnan(f.agl_fused_m) && f.agl_fused_m >= FC_LIFTOFF_MIN_AGL_M) liftoff_cond = true;
  ...
  switch (s_state) {
    case FC_PREFLIGHT:
      if (tilt_latched) { s_state = FC_ABORT_LOCKOUT; t_state_ms = now_ms; break; }
      // Stay in preflight until liftoff
      if (liftoff_latched) { s_state = FC_BOOST; t_state_ms = now_ms; }
      break;
    case FC_BOOST:
      if (tilt_latched) { s_state = FC_ABORT_LOCKOUT; t_state_ms = now_ms; break; }
      if (burnout_latched) { s_state = FC_POST_BURN_HOLD; t_state_ms = now_ms; }
      break;
    case FC_POST_BURN_HOLD:
      if (tilt_latched) { s_state = FC_ABORT_LOCKOUT; t_state_ms = now_ms; break; }
      if (now_ms - t_state_ms >= FC_BURNOUT_HOLD_MS) { s_state = FC_WINDOW; t_state_ms = now_ms; }
      break;
    case FC_WINDOW: {
      if (tilt_latched) { s_state = FC_ABORT_LOCKOUT; t_state_ms = now_ms; break; }
    ...
    case FC_DEPLOYED:
      ...
      if (!isnan(f.t_apogee_s) && f.t_apogee_s <= FC_RETRACT_BEFORE_APOGEE_S) {
        s_state = FC_RETRACTING; t_state_ms = now_ms;
    ...
    case FC_RETRACTING:
      ...
      s_state = FC_LOCKED; t_state_ms = now_ms;
      break;
    case FC_LOCKED:
      // Do nothing
      break;
    case FC_ABORT_LOCKOUT:
      ...
      break;
    default:
      s_state = FC_SAFE; t_state_ms = now_ms; break;
  }
  ...
  switch (s_state) {
    case FC_DEPLOYED:
      cmd_deg = FC_DEPLOY_CMD_DEG; break;
    case FC_RETRACTING:
    case FC_LOCKED:
    case FC_ABORT_LOCKOUT:
    case FC_BOOST:
...
static void fc_task(void *param) {
  ...
  for (;;) {
    ...
    svc::fusionGetAlt(f);
    uint32_t now = millis();
    uint32_t dt = now - prev_ms; if (dt > 1000) dt = 1000; if (dt < 1) dt = 1;
    prev_ms = now;
