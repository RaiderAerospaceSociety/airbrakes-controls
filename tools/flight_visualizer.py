...
class HBarGauge:
    ...
    def __init__(self, ax: plt.Axes, vmin: float, vmax: float, label: str, unit: str = "") -> None:
        ...
        # Background bar
        self.bg = ax.barh(0.5, width=(self.vmax - self.vmin), left=self.vmin, height=0.6, color="#e0e0e0")
...
class Compass:
    ...
    def __init__(self, ax: plt.Axes, label: str = "Azimuth/ Tilt") -> None:
        ...
        ax.plot(np.cos(theta), np.sin(theta), color="#444444", lw=2)
        # Cardinal labels
...
class FlightVisualizer:
    def __init__(self, ser: serial.Serial, window: int, *, show_raw: bool, raw_buffer: int, print_raw: bool) -> None:
        ...
        rows = 6 + 1  # top, lights, dial, timeseries, compass, spacer, raw
        gs = GridSpec(rows, 1, height_ratios=[1.2, 0.9, 1.4, 2.4, 1.6, 0.2, 1.0], figure=self.fig)
        ...
        # Airbrake dial
        self.ax_brake = self.fig.add_subplot(gs[2, 0])
        ...
        # Compass row
        self.ax_compass = self.fig.add_subplot(gs[4, 0])

        # Spacer row (gs[5])

        # Raw row
        self.ax_raw = self.fig.add_subplot(gs[6, 0])
        ...
        # Temperature gauge (Celsius, general purpose range)
        self.temp_gauge = HBarGauge(self.ax_temp, -10.0, 80.0, label="Temp (C)", unit="°C")
    ...
    def _update(self, _frame: int):
        ...
        # Temperature
        self.temp_gauge.update(self._get_float(last.get("temp_c")))

        # State and lockout
        st = str(last.get("fc_state_str", "")) if last.get("fc_state_str") is not None else ""
        state_txt = st if st else "--"
        self.state_txt.set_text(f"STATE: {state_txt}")
        lockout = flags.get("lockout")
        self.lockout_txt.set_text(f"LOCKOUT: {'ON' if lockout else ('OFF' if lockout is not None else '--')}")
        self.lockout_txt.set_color("#d62728" if lockout else ("#2ca02c" if lockout is not None else "#444444"))
        ...
        self.clock_alive.set_text(f"Alive: {fmt_time(t_alive)}")
        self.clock_since.set_text(f"Since liftoff: {fmt_time(t_since)}")
        self.clock_to_ap.set_text(f"To apogee: {fmt_time(t_to_ap)}")

        # Errors
        i2c_errs = self._get_int(last.get("i2c_errs"))
        spi_errs = self._get_int(last.get("spi_errs"))
        self.err_i2c.set_text(f"i2c_errs: {i2c_errs if i2c_errs is not None else '--'}")
        self.err_spi.set_text(f"spi_errs: {spi_errs if spi_errs is not None else '--'}")
